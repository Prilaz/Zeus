...
import { addProduct, getAllCategories } from "../../../api/prouductAPI";
import Swal from "sweetalert2";

const AddProduct = () => {
  let [product, setProduct] = useState({});
  let [categories, setCategories] = useState([]);
  let { product_name, product_price, product_description, count_in_stock } =
    product;
  ...
  const handleSubmit = (e) => {
    ...
    for (var key in product) {
      productFormData.set(key, product[key]);
    }
    addProduct(productFormData).then((data) => {
      if (data.error) {
